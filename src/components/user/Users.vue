  ...
  <div>
    ...
    <el-dialog title="分配角色" :visible.sync="setRolesDialogVisible" width="50%" @close="setRolesDialogClosed">
        ...
        <span slot="footer" class="dialog-footer">
            ...
            <el-button type="primary" @click="savrRoleInfo">确 定</el-button>
        </span>
    </el-dialog>
  </div>
</template>
...
export default {
    ...
    methods:
    {
        async getUserList()
        {
            const {data:res}=await this.$https.get('users',{params:this.queryInfo});
        ...
        {
            const {data:res}=await this.$https.put(`users/${userInfo.id}/state/${userInfo.mg_state}`);
            if(res.meta.status!==200)
            {
        ...
        addUser()
        {
            this.$refs.addFormRef.validate(async valid=>
            {
                ...
                //可以发起添加用户的添加请求
                const {data:res}=await this.$https.post('users',this.addForm);
        ...
        editUser()
        {
            this.$refs.editFormRef.validate(async valid=>
            {
                ...
                //可以发起修改用户的添加请求
                const {data:res}=await this.$https.put('users/'+this.editForm.id,
        ...
        async savrRoleInfo()
        {
            ...
            const {data:res}=await this.$https.put(`users/${this.userInfo.id}/role`,{rid:this.selectedRoleId});
            if(res.meta.status!==200)
            {
